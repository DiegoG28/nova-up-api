...
export class StatusPostEnumChanged1693244987135 implements MigrationInterface {
    name = 'StatusPostEnumChanged1693244987135'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`recurso\` DROP FOREIGN KEY \`FK_0271858854b96137befdd4e1963\``);
        await queryRunner.query(`ALTER TABLE \`recurso\` CHANGE \`hash\` \`hash\` varchar(64) NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`publicacion\` CHANGE \`status\` \`status\` enum ('aprobado', 'pendiente', 'rechazado') NOT NULL DEFAULT 'pendiente'`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`publicacion\` CHANGE \`fecha\` \`fecha\` datetime NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`publicacion\` CHANGE \`status\` \`status\` enum COLLATE "utf8mb4_general_ci" ('aprobado', 'pendiente', 'cancelado') NOT NULL DEFAULT ''pendiente''`);
        await queryRunner.query(`ALTER TABLE \`publicacion\` ADD CONSTRAINT \`FK_b0cd676ac183ff45644252a0265\` FOREIGN KEY (\`id_usuario\`) REFERENCES \`usuario\`(\`id_usuario\`) ON DELETE SET NULL ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`publicacion\` ADD CONSTRAINT \`FK_b62df2c1cdb9126f37647d90025\` FOREIGN KEY (\`id_categoria\`) REFERENCES \`categoria\`(\`id_categoria\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`recurso\` CHANGE \`hash\` \`hash\` varchar(64) COLLATE "utf8mb4_general_ci" NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`recurso\` ADD CONSTRAINT \`FK_0271858854b96137befdd4e1963\` FOREIGN KEY (\`id_publicacion\`) REFERENCES \`publicacion\`(\`id_publicacion\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

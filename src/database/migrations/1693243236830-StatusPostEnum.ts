...
export class StatusPostEnum1693243236830 implements MigrationInterface {
    name = 'StatusPostEnum1693243236830'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`publicacion\` ADD \`status\` enum ('aprobado', 'pendiente', 'cancelado') NOT NULL DEFAULT 'pendiente'`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`recurso\` ADD CONSTRAINT \`FK_0271858854b96137befdd4e1963\` FOREIGN KEY (\`id_publicacion\`) REFERENCES \`publicacion\`(\`id_publicacion\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`publicacion\` DROP COLUMN \`status\``);
    }
